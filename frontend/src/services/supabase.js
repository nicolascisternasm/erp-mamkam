...
const toCot  = (c, eId) => ({ id: c.id, empresa_id: eId, numero: c.numero, fecha: c.fecha, cliente: c.cliente, comuna: c.comuna, direccion: c.direccion, telefono: c.telefono, email: c.email, estado: c.estado, observaciones: c.observaciones, enviado_whatsapp: c.enviadoWhatsapp, enviado_email: c.enviadoEmail, items: c.items, neto: c.neto, iva: c.iva, total: c.total, condiciones_pago: c.condicionesPago?.length ? c.condicionesPago : null, glosa: c.glosa || null, fecha_expiracion: c.fechaExpiracion || null, descuento_tipo: c.descuentoTipo ?? 'porcentaje', descuento_valor: c.descuentoValor ?? 0, usuario_id: c.usuarioId || null, creado_por: c.creadoPor || null, estado_total: c.estadoTotal ?? 'pendiente', productos_asociados: c.productos_asociados ?? [], pagos_comprobantes: c.pagosComprobantes?.length ? c.pagosComprobantes : null, ordenes_compra_cliente: c.ordenesCompraCliente?.length ? c.ordenesCompraCliente : null })
