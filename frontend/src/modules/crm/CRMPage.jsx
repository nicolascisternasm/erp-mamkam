...
const formatWhatsappLink = (telefono) => {
  ...
  const num = String(telefono).replace(/[\s\-()+]/g, '')
  if (!num) return null
  const limpio = num.length === 9 && !num.startsWith('56') ? '56' + num : num
...
function KanbanCard({ cliente, onClick }) {
  ...
  return (
    ...
    >
      ...
      {cliente.telefono && formatWhatsappLink(cliente.telefono) && (
        <a
              ...
              <div className="space-y-2">
                ...
                {sel.telefono && (
                  <div className="flex items-center gap-3">
                    ...
                    {formatWhatsappLink(sel.telefono) && (
                      <a
                        href={formatWhatsappLink(sel.telefono)}
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-1 text-xs text-green-600 hover:text-green-700 bg-green-50 hover:bg-green-100 rounded-full px-2 py-0.5 transition-colors"
                      >
